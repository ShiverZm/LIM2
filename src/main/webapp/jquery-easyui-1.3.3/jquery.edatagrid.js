...
(function($) {
					...
					if (state) {
						...
						$.data(this, 'edatagrid', {
									options : $.extend({},
											...
											$.fn.edatagrid.parseOptions(this),
											options)
								});
					}
					buildGrid(this);
	...
	$.fn.edatagrid.methods = {
		...
		addRow : function(jq) {
			return jq.each(function() {
						...
						if (opts.tree) {
							var node = $(opts.tree).tree('getSelected');
							rows[opts.editIndex][opts.treeParentField] = (node
									? node.id
									: 0);
						}
	...
	$.fn.edatagrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
				...
				treeDndUrl : null, // to process the drag and drop operation,
									// return {success:true}
				treeTextField : 'name',
